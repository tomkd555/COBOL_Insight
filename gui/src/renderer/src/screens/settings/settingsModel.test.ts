...
describe("ルールカタログの読み出し", () => {
  it("バグ検出 31 件と SQL 助言 6 件の合計 37 件を扱う", () => {
    expect(RULE_TOTAL).toBe(37);
    expect(RULE_IDS.filter((id) => id.startsWith("R"))).toHaveLength(31);
    expect(RULE_IDS.filter((id) => id.startsWith("S"))).toHaveLength(6);
  });

  it("ID はカタログの定義順(R001→R031→S001→S006)である", () => {
    expect(RULE_IDS[0]).toBe("R001");
    expect(RULE_IDS[30]).toBe("R031");
    expect(RULE_IDS[36]).toBe("S006");
  });
...
describe("buildRuleGroups", () => {
  it("検索語が無ければ全件をカテゴリ別にまとめる", () => {
    ...
    expect(groupedRuleIds(groups)).toHaveLength(RULE_TOTAL);
