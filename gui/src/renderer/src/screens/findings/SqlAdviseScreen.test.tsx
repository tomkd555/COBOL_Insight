...
/** 解析実行が sql-advise の SARIF を収めた状態。 */
...
describe("SqlAdviseScreen(SQL助言)", () => {
  it("empty では「表示できる SQL がありません」を出し、sql-advise は起動しない", () => {
    renderSql(emptySeed);
    expect(screen.getByRole("region", { name: "表示できる SQL がありません" })).toBeInTheDocument();
    expect(runSqlAdvise).not.toHaveBeenCalled();
  });

  it("解析実行が収めた SARIF から S001〜S006 を一覧化し、sql-advise を再起動しない(ゲート経路)", () => {
    renderSql(resultsSeed);
    expect(dataRows()).toHaveLength(6);
    expect(screen.getByRole("table", { name: "SQL 助言一覧" })).toBeInTheDocument();
    expect(runSqlAdvise).not.toHaveBeenCalled();
  ...
  it("sql-advise が失敗した場合は 0 件と見せず、失敗と理由を示す", () => {
    renderSql({
      ...resultsSeed,
      mode: "error",
      sqlAdvice: { status: "error", message: "sql-advise が SARIF を出力しませんでした。" },
    });
    const region = screen.getByRole("region", { name: "SQL 助言を取得できませんでした" });
    expect(region).toHaveTextContent("sql-advise が SARIF を出力しませんでした。");
  ...
  it("SARIF を未取得のまま解析済みになった場合は未取得として示す", () => {
    ...
    expect(screen.getByRole("region", { name: "SQL 助言をまだ取得していません" })).toBeInTheDocument();
  });
  ...
  it("フィルタはタブを移動して戻っても保たれる", () => {
    ...
    fireEvent.change(screen.getByRole("combobox", { name: "ルールで絞り込み" }), { target: { value: "S002" } });
    expect(dataRows()).toHaveLength(1);

    fireEvent.click(screen.getByRole("button", { name: "指摘一覧タブ" }));
    fireEvent.click(screen.getByRole("button", { name: "SQL助言タブ" }));
    expect(screen.getByRole("combobox", { name: "ルールで絞り込み" })).toHaveValue("S002");
    expect(dataRows()).toHaveLength(1);
  });
  ...
  it("行を選ぶと原本から SQL 本文を読んで詳細ペインへ出す", async () => {
    ...
    const detail = within(screen.getByRole("complementary", { name: "SQL 文と最適化助言の詳細" }));
  ...
  it("詳細ペインは同じ位置の助言を全件示す(1 つの SQL 文に複数の助言)", async () => {
    ...
    fireEvent.click(screen.getByRole("row", { name: /^S001 / }));
    const detail = within(screen.getByRole("complementary", { name: "SQL 文と最適化助言の詳細" }));
    expect(await detail.findByText("最適化助言 3 件")).toBeInTheDocument();
  ...
  it("同一位置に同一ルールの助言が2件あっても key が衝突せず両方を描く", async () => {
    // 1 行に EXEC SQL 文が2つ並ぶと、助言の位置(文の開始行・1桁)が一致して ruleId まで重なる。
    ...
    const detail = within(screen.getByRole("complementary", { name: "SQL 文と最適化助言の詳細" }));
    expect(await detail.findByText("最適化助言 2 件")).toBeInTheDocument();
