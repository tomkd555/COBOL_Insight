 ...
 *
 * レポートの本文は GUI で組み立てない(裁定 A1)。`report` は1回の実行で HTML(--html)とテキスト
...
export function exitCodeWarning(exitCode: number): string | null {
  ...
  if (exitCode === 1) {
    return "警告のある指摘を含みます。レポート本体は書き出されています。";
  }
  return "エラーのある指摘、または解析の失敗を含みます。レポート本体は書き出されていますが、対象資産の一部が解析できていない可能性があります。";
}
