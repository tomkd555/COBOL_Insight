...
/**
 * 埋め込みSQL文の構文レベルの構造シグナル。SQL指摘ルール(S001・S002・S004)が真偽値・リストで読む。
 ...
 * @param functionOnColumnPredicates 比較の片側が列引数の関数呼出しまたは CAST(col AS ..) の該当箇所(S002)
 * @param cursor                     DECLARE CURSOR の場合のカーソル情報。それ以外は空(S004)
 * @param hasFetchFirst              FETCH FIRST n ROWS ONLY 句の有無(参考)
 * @param hasOptimizeFor             OPTIMIZE FOR n ROWS 句の有無(参考)
