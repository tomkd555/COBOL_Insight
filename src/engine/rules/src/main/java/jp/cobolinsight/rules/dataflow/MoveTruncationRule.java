...
public final class MoveTruncationRule implements Rule {
    ...
    public RuleDoc doc() {
        return RuleDoc.named("MOVEによる桁落ち・切り捨て", "データ移動")
                .summary("受信項目の桁数・文字長が送信項目より小さい MOVE を検出する。")
                .rationale("数値では上位桁が、英数字では末尾の文字が失われる。"
                        + "実行時の異常にはならないため、金額や識別子が黙って別の値になる。")
                .detection("送受信の PICTURE を解決し、数値項目どうしで受信の整数部または小数部が"
                        + "送信より短いもの、英数字項目どうしで送信が受信より長いものを検出する。"
                        + "図形定数・文字列リテラル・集団項目・参照修飾を送信に含む MOVE と、"
                        + "送受信の種別が異なる MOVE は対象外とする。")
                .remedy("受信項目の PICTURE を送信項目以上に広げる。"
                        + "切り捨てが意図なら、参照修飾で切り出す範囲を明示する。")
                .example("""
