	...
	*/
	public void resolveParms(ArrayList<PPSetSymbolValue> setSym) {
		...
		if (this.isExecPgm()) {
			...
			/*
			Local patch to the upstream defect: the EXEC statement's actual proc
			parameters (this.setSym) may themselves be symbolic (e.g. CYCLE=&CYCLE
			with a preceding SET CYCLE=250718).  Without resolving them here,
			PPSymbolic.getResolvedText() substitutes the literal "&CYCLE" into the
			proc being executed.
			*/
			for (PPSetSymbolValue s: this.setSym) {
				s.resolveParms(setSym);
			}
			this.procExecuted.resolveParms(setSym);
			for (PPDdStatementAmalgamation dda: ddStatements) {
