...
// 実際の取り込みはカタログ(モジュール共通の状態)を置き換えるため、他のテストへ影響しないよう
// 差し替える。ここで確かめたいのは「保存の後に取り直しを呼ぶか」であって取り込みの中身ではない。
vi.mock("../../data/loadRuleCatalog", () => ({
  loadRuleCatalog: vi.fn().mockResolvedValue(undefined),
}));

let checkDirectoryExists: ReturnType<typeof vi.fn>;
let writeUserRules: ReturnType<typeof vi.fn>;
...
describe("SettingsScreen のコピー句探索パス", () => {
  ...
  it("追加すると末尾へ加わり、入力欄が空になる", () => {
    renderSettings(seedState());
    const input = screen.getByLabelText("追加するパス");
  ...
  it("重複するパスは加えない", () => {
    renderSettings(seedState());
    fireEvent.change(screen.getByLabelText("追加するパス"), {
...
describe("ルールの説明", () => {
  it("説明を開くと、検出条件・理由・対処・例を示す", () => {
    renderSettings(seedState({ ruleSearch: "R003" }));
    fireEvent.click(screen.getByRole("button", { name: "R003 MOVEによる桁落ち・切り捨て の説明" }));
    expect(screen.getByText("何を検出するか")).toBeInTheDocument();
    expect(screen.getByText("なぜ問題か")).toBeInTheDocument();
    expect(screen.getByText("検出条件")).toBeInTheDocument();
    expect(screen.getByText("どう直すか")).toBeInTheDocument();
    expect(screen.getByText("該当する例")).toBeInTheDocument();
    expect(screen.getByText("直した例")).toBeInTheDocument();
  });

  it("説明の開閉を aria-expanded で示し、もう一度押すと閉じる", () => {
    renderSettings(seedState({ ruleSearch: "R003" }));
    const toggle = screen.getByRole("button", { name: "R003 MOVEによる桁落ち・切り捨て の説明" });
    expect(toggle).toHaveAttribute("aria-expanded", "false");
    fireEvent.click(toggle);
    expect(toggle).toHaveAttribute("aria-expanded", "true");
    fireEvent.click(toggle);
    expect(toggle).toHaveAttribute("aria-expanded", "false");
    expect(screen.queryByText("検出条件")).toBeNull();
  });

  it("一度に開くのは1件とし、別の行を開くと前の説明を閉じる", () => {
    renderSettings(seedState({ ruleSearch: "R00" }));
    const first = screen.getByRole("button", { name: /^R001 .+ の説明$/ });
    const second = screen.getByRole("button", { name: /^R002 .+ の説明$/ });
...
describe("利用者定義ルール", () => {
  ...
  it("1件も無ければ作り方を示す", () => {
    renderSettings(seedState());
    expect(screen.getByText(/利用者定義ルールはまだ1件もない/)).toBeInTheDocument();
  });

  it("定義した内容を一覧へ並べる", () => {
    renderSettings(seedState({ userRules: [EXISTING] }));
    expect(screen.getByText("コンソール入力の使用")).toBeInTheDocument();
    expect(screen.getByText("FROM\\s+CONSOLE")).toBeInTheDocument();
    expect(screen.getByText(/COBOL \/ 重大度 高/)).toBeInTheDocument();
  });

  it("追加すると ID を自動で採り、必須の欄が空のうちは保存できない", () => {
    renderSettings(seedState({ userRules: [EXISTING] }));
    fireEvent.click(screen.getByRole("button", { name: "＋ ルールを追加" }));
    expect(screen.getByLabelText("ID")).toHaveValue("U002");
    expect(screen.getByRole("button", { name: "追加する" })).toBeDisabled();
  });

  it("入力した正規表現を試験本文へ当て、一致した行を示す", () => {
    renderSettings(seedState());
    fireEvent.click(screen.getByRole("button", { name: "＋ ルールを追加" }));
    fireEvent.change(screen.getByLabelText("正規表現"), { target: { value: "GOBACK" } });
    fireEvent.change(screen.getByLabelText("試験する本文"), {
      target: { value: "000100     GOBACK." },
    });
    expect(screen.getByText("1 行を走査し、1 件が一致した。")).toBeInTheDocument();
    expect(screen.getByText("1 行 12 桁")).toBeInTheDocument();
  });

  it("解釈できない正規表現は理由を示し、保存を許さない", () => {
    renderSettings(seedState());
    fireEvent.click(screen.getByRole("button", { name: "＋ ルールを追加" }));
    fireEvent.change(screen.getByLabelText("正規表現"), { target: { value: "A(" } });
    expect(screen.getByRole("alert")).toHaveTextContent("正規表現を解釈できない");
    expect(screen.getByRole("button", { name: "追加する" })).toBeDisabled();
  });
  ...
  it("既存と同じ ID を拒む", () => {
    renderSettings(seedState({ userRules: [EXISTING] }));
    fireEvent.click(screen.getByRole("button", { name: "＋ ルールを追加" }));
    fireEvent.change(screen.getByLabelText("ID"), { target: { value: "U001" } });
    expect(screen.getByRole("alert")).toHaveTextContent("既に使われている");
  });

  it("保存すると定義ファイルへ書き、カタログを取り直す", async () => {
    const { loadRuleCatalog } = await import("../../data/loadRuleCatalog");
    renderSettings(seedState());
    fireEvent.click(screen.getByRole("button", { name: "＋ ルールを追加" }));
    fireEvent.change(screen.getByLabelText("名称"), { target: { value: "自社で禁じた命令" } });
    fireEvent.change(screen.getByLabelText("正規表現"), { target: { value: "ALTER" } });
    fireEvent.change(screen.getByLabelText("指摘のメッセージ"), { target: { value: "禁止" } });
    fireEvent.click(screen.getByRole("button", { name: "追加する" }));
    await waitFor(() => expect(writeUserRules).toHaveBeenCalled());
    const [path, file] = writeUserRules.mock.calls[0];
    expect(path).toBe("C:\\data\\user-rules.json");
    expect(file.rules).toHaveLength(1);
    expect(file.rules[0].id).toBe("U001");
    expect(file.rules[0].pattern).toBe("ALTER");
    expect(loadRuleCatalog).toHaveBeenCalled();
    await waitFor(() =>
      expect(screen.getByTestId("toast-msg")).toHaveTextContent("U001 を保存しました。"),
    );
  });

  it("削除すると残りの定義だけを書き戻す", async () => {
    renderSettings(seedState({ userRules: [EXISTING] }));
    fireEvent.click(screen.getByRole("button", { name: "削除" }));
    await waitFor(() => expect(writeUserRules).toHaveBeenCalled());
    expect(writeUserRules.mock.calls[0][1].rules).toEqual([]);
  });

  it("解析の実行中は追加も編集もできない", () => {
    renderSettings(seedState({ mode: "running", userRules: [EXISTING] }));
    expect(screen.getByRole("button", { name: "＋ ルールを追加" })).toBeDisabled();
    expect(screen.getByRole("button", { name: "編集" })).toBeDisabled();
  });

  it("engine が返した定義の誤りを示す", () => {
    renderSettings(seedState({ userRuleErrors: ["rules[0]: id が無い、または空である"] }));
    expect(screen.getByRole("alert")).toHaveTextContent("id が無い、または空である");
  });
});
