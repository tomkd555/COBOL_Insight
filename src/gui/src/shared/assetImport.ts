/**
 * 資産フォルダへ取り込むファイルの置き場所の規約。engine の走査は資産フォルダ直下の
 * bms・cobol・copy・copybook・jcl を見て、種別ごとに定めた拡張子のファイルを拾う。コピー句は
 * copy・copybook のいずれでも拾うため、取込先は copy にそろえる。取込先の決定は書き込む main と、
 * 保存先を示す renderer の双方が要するため、共有層へ置く。
 */
