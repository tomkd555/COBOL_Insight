...
/*
Local patch to the upstream defect: with [\n\r] (single character) a CRLF line
terminator is split.  Rules such as "COMMA_DFLT NEWLINE" then consume only the
CR and switch to a continuation mode (e.g. COMMA_NEWLINE_MODE) where the
leftover LF becomes an ERROR_CHAR token, which terminates the jobCard rule at
its first line.
*/
NEWLINE
	: ('\r'? '\n' | '\r')
...
/*
Local patch to the upstream defect: with [\n\r] (single character) a CRLF line
terminator is split, the leftover LF becoming an ERROR_CHAR in QS_SS_MODE and
breaking quoted strings continued across lines.
*/
QS_NEWLINE
	: ('\r'? '\n' | '\r')
